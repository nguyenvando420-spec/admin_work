...
def test_user_with_view_and_fore_can_run(client, setup_test_data):
    resp = client.post("/api/v1/auth/login", data={"username": "user_op", "password": setup_test_data["pass"]})
    token = resp.json()["access_token"]
...
def test_user_with_fore_but_no_view_cannot_run(client, setup_test_data):
    resp = client.post("/api/v1/auth/login", data={"username": "user_bad_op", "password": setup_test_data["pass"]})
    token = resp.json()["access_token"]
...
def test_user_with_no_roles_cannot_access(client, setup_test_data):
    resp = client.post("/api/v1/auth/login", data={"username": "user_noroles", "password": setup_test_data["pass"]})
    token = resp.json()["access_token"]
